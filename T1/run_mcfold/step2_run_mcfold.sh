...
echo "./mcfold.static.exe >seq${count}_5bp_267_GCstem_3nt_bulges.data" >>seq${count}_run_mcfold.sh

chmod 771 seq${count}_run_mcfold.sh

sed -ig 's/pass/"pass/g' seq${count}_run_mcfold.sh

sed -ig 's/top=100/top=100"/g' seq${count}_run_mcfold.sh
